//! Domain policy for Activity aggregate
//!
...
/// Domain policy for Activity — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<Activity>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type ActivityDomainPolicy = PermitAllPolicy<Activity>;
