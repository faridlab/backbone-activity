...
--
-- Strip every company-fence artifact from the activity table (ADR-0029): the module is
-- tenant-agnostic; org scoping is installed by the COMPOSING service's tenancy decorator,
-- never by the module. Dropped here: the company-leading index, the
-- activities_company_isolation RLS policy, and the company_id column itself.
--
...
DECLARE
    ...
    offenders text := '';
BEGIN
    FOREACH t IN ARRAY ARRAY['activities']
    LOOP
        IF to_regclass(format('activity.%I', t)) IS NULL THEN
            CONTINUE; -- chain not fully applied on this database; nothing to strip
        END IF;

        SELECT EXISTS (
                   SELECT 1 FROM information_schema.columns
                   WHERE table_schema = 'activity' AND table_name = t AND column_name = 'org_unit_id'
               )
        INTO has_org;

        EXECUTE format('SELECT count(*) FROM activity.%I', t) INTO total;

        IF has_org THEN
            EXECUTE format(
                'SELECT count(*) FROM activity.%I WHERE org_unit_id IS NULL', t)
            INTO org_nulls;
        ELSE
        ...
        END IF;
        offenders := offenders || format(' activity.%s (%s rows, %s rows not covered by org_unit_id);', t, total, org_nulls);
    END LOOP;
...
-- ── activities ────────────────────────────────────────────────────────────────
DROP INDEX IF EXISTS activity.idx_activities_company_id_status;
DROP POLICY IF EXISTS activities_company_isolation ON activity.activities;
ALTER TABLE activity.activities DROP COLUMN IF EXISTS company_id;
