...
metaphor migration generate Invoice activity
metaphor migration run
...
(`activity` is your module name — auto-detected from the current directory when omitted, but
...
```bash
metaphor migration seed activity          # run Rust seeders
metaphor migration generate-seeds activity  # emit SQL seed files
```
